...
class TestClass:
    def test_1a(self):
        ...
        assert input_1.match_patterns() == input_2.match_patterns() == {'pat_1': ['allow', 'registering'], 'pat_2': ['arbitrary', 'search']}


    def test_two(self):
        # Test on more than one pattern
        input = Match('Allow for registering an arbitrary number of pattern groups to search input text')
        t_patterns = {'pat_1': ['or', 'ext'], 'pat_2': ['ow'], 'pat_3': ['an', 'er', 'ern']}
        input.add_patterns(t_patterns)

        assert input.match_patterns('pat_1') == {'pat_1': ['for', 'text']} \
               and input.match_patterns('pat_2') == {'pat_2': ['allow']} \
               and input.match_patterns('pat_3') == {'pat_3': ['registering','an', 'number', 'pattern' ]}



    def test_three(self):
        # Specify a specific patter group
        input = Match('Allow for registering an arbitrary number of pattern groups to search input text')
        t_patterns = {'pat_1': ['or', 'ext'], 'pat_2': ['ow'], 'pat_3': ['an', 'er', 'ern']}
        input.add_patterns(t_patterns)
        assert input.match_patterns(['pat_1', 'pat_3']) \
